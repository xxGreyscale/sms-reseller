package com.smsreseller.payment.presentation;

import com.smsreseller.payment.application.BundleAdminService;
...
public class AdminBundleController {
    ...
    public ResponseEntity<BundleDto> create(@Valid @RequestBody BundleSaveRequest req) {
        SmsBundle saved = bundleAdminService.create(req.name(), req.smsCount(), req.priceTzs(), req.active());
    ...
    public ResponseEntity<BundleDto> update(@PathVariable UUID id,
        ...
        try {
            SmsBundle updated = bundleAdminService.update(id, req.name(), req.smsCount(), req.priceTzs(), req.active());

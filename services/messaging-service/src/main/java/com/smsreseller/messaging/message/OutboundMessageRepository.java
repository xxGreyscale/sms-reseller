...
import com.smsreseller.messaging.analytics.OperatorRateDto;
...
public interface OutboundMessageRepository extends JpaRepository<OutboundMessage, UUID> {
     ...
     */
    @Query("""
        SELECT new com.smsreseller.messaging.analytics.OperatorRateDto(m.operator, CAST(m.status AS string), COUNT(m))

...
**Goal**: External procurement runs as a background track in parallel with all coding phases â none of phases 1â6 wait for it
**Depends on**: Nothing â runs concurrently with all other phases
**Requirements**: None (all external procurement actions)
**Note**: Coding is never blocked by this phase. Stub implementations cover NIDA, Azampay, and the SMS provider until real credentials arrive. When they do, swap one Spring `@Profile` bean â no application logic changes.
**Success Criteria** (what must be TRUE when complete):
  ...
  3. Upstream SMS provider selected with signed wholesale rate (~9â11 TZS/SMS), numeric shortcode allocated, DLR webhook contract and `clientRef` idempotency support confirmed
...
**Depends on**: Nothing â this is the unblocked starting point for coding
...
**Goal**: A verified user can register, complete async NIDA verification, log in, manage their session, reset a forgotten password, and be assigned a sender ID â and every other module can trust the JWTs this module issues
...
**Depends on**: Phase 1
**Mock-first**: `NidaVerificationService` interface built with two implementations â `StubNidaVerificationService` (auto-verifies after 3s delay, active in dev/staging via `@Profile("stub")`) and `RealNidaVerificationService` (wired via `@Profile("prod")` when access arrives). Full NIDA flow is testable and demoable from day one.
...
**Success Criteria** (what must be TRUE):
  ...
  5. All 8 downstream modules can validate the JWT issued by this module using the shared-security library â no service needs to call identity at runtime for token validation
**Plans**: 6 plans (5 waves)
- [x] 02-01-PLAN.md â Wave 0: test infra (Testcontainers PG16+Redis7), build deps, RSA fixture, one plain-stub test per requirement + cross-module JWT contract
- [x] 02-02-PLAN.md â Wave 1: asymmetric JWT core (issuer + shared-security validator), User aggregate + V1 migration, security/redis/async config
- [x] 02-03-PLAN.md â Wave 2: registration (PENDING immediately), async NIDA stub/real + degraded retry, VerificationFinalizer seam (IDEN-01/02/08)
- [x] 02-04-PLAN.md â Wave 2: email+password login, opaque refresh tokens in Redis (rotation + reuse detection), lockout, logout revoke-current (IDEN-04/05/06)
- [x] 02-06-PLAN.md â Wave 3: verification finalize TX (VERIFIED flip + numeric sender ID + UserVerified(50) outbox + RabbitMQ relay) (IDEN-03, SNDR-01)
- [x] 02-05-PLAN.md â Wave 4: password reset via email link (single-use TTL token, mock-first email), revoke-all sessions (IDEN-07)
**UI hint**: no
...
**Success Criteria** (what must be TRUE):
  1. User can view available bundle catalog (Taster FREE / Starter / Growth / Pro / Scale) and initiate a purchase â the STK push appears on their phone within 5 seconds and a 2-minute countdown is shown
  ...
  3. If the STK push times out or the user declines, the payment is marked EXPIRED/FAILED and the user sees a clear error â no infinite spinner
...
- [x] 03-01-PLAN.md â Wave 0: both-service build deps + Testcontainers bases (PG16/Redis/RabbitMQ) + 14 placeholder ITs (one per requirement)
- [x] 03-02-PLAN.md â Wave 1: wallet ledger core â lot-based append-only credit_lots/credit_transactions, derived balance, expiry-soonest-first SELECT FOR UPDATE reservation (WLET-02/03/06/07)
- [x] 03-03-PLAN.md â Wave 1: payment foundation â Flyway-seeded bundle catalog + read API, Payment entity/state-machine + single-pending index, PaymentGateway stub, security/AMQP config (PYMT-01)
- [x] 03-04-PLAN.md â Wave 2: wallet UserVerified consumer (idempotent 50-credit bonus grant) + balance/transaction-history API (WLET-01/02)
- [x] 03-05-PLAN.md â Wave 2: payment flow â purchase initiation (single-pending), idempotent callback (PENDING+EXPIREDâSUCCESS), 2-min EXPIRED sweep, PaymentConfirmed outbox, history (PYMT-02/03/04/05/06/07)
- [x] 03-06-PLAN.md â Wave 3: wallet PaymentConfirmed consumer (12-mo purchased grant) + idempotent refund mechanism + low-credit/expiry-warning sweeps + Azampay reconciliation + prod gateway (PYMT-08, WLET-04/05)
**UI hint**: no
...
**Success Criteria** (what must be TRUE):
  ...
  3. User can create and send an immediate bulk SMS campaign â credits are reserved (not deducted) before the campaign reaches QUEUED state, the campaign is refused with a clear error if credits are insufficient
...
- [x] 04-01-PLAN.md â Wave 0: contact + messaging module deps (commons-csv, libphonenumber), Testcontainers IT bases, one placeholder failing test per requirement
- [x] 04-02-PLAN.md â Wave 1: contact-service core â Contact/Group/Suppression entities + migrations + CRUD API + IDOR guard (CONT-01/02/03/04/08)
- [x] 04-04-PLAN.md â Wave 1: messaging foundation â campaign/message/sender-id entities + migrations, SmsProvider stub, GSM-7/UCS-2 encoder, quorum+DLX queue topology, outbox (MESG-01/02)
- [x] 04-07-PLAN.md â Wave 1: wallet additions â per-lot allocation in ReservationResult, consumeFromLot/releaseFromLot, idempotent MessagingEventConsumer (CONSUME/RELEASE/REFUND) (MESG-10 wallet side)
- [x] 04-03-PLAN.md â Wave 2: CSV import â libphonenumber E.164 normalization + dedup + import summary (CONT-05/06/07/09)
- [x] 04-05-PLAN.md â Wave 2: send pipeline â recipient expansion + suppression, sync reserve-before-QUEUED, per-message lot snapshot, publish, SendMessageConsumer acceptâCONSUME/releaseâRELEASE (MESG-03/08/09)
- [x] 04-06-PLAN.md â Wave 3: DLX retry ladder + DeadLetterConsumer refund + delivery-receipt ingestion + per-message/aggregate status (MESG-06/07/10)
- [x] 04-08-PLAN.md â Wave 4: scheduled dispatch + cancel + sender-ID request/approve/reject state machine + SenderIdDecided event (MESG-04/05, SNDR-02/03/04)
**UI hint**: no
...
**Success Criteria** (what must be TRUE):
  1. Users receive in-app notifications for all key events: NIDA verified, payment confirmed, low-credit alert, credit expiry warning (7-day), campaign completion summary, and sender ID decision â each fires exactly once regardless of event replay
...
- [x] 05-01-PLAN.md â Wave 0: notification + admin Spring modules + Testcontainers bases, 15 RED placeholder ITs (all 16 reqs), admin-web Next.js 14 scaffold + Tailwind 3 + shadcn 3.5 + Vitest/Playwright harness + Dockerfile
- [x] 05-02-PLAN.md â Wave 1: messaging-service â D-12 CampaignCompleted outbox emit + D-13 operator column/CarrierResolver, ANLX-01/03 analytics (NOTF-05 prereq)
- [x] 05-03-PLAN.md â Wave 1: identity-service â issueAdminToken (60-min ROLE_ADMIN), admin login + seeded admin (Flyway placeholder), user search (ADMN-01/02)
- [x] 05-04-PLAN.md â Wave 1: wallet-service â admin ledger inspection (ADMN-03), credit-usage trend (ANLX-02), admin-reachable refund (ADMN-05)
- [x] 05-05-PLAN.md â Wave 1: payment-service â ADMIN bundle catalog CRUD + validation (ADMN-07)
- [x] 05-06-PLAN.md â Wave 2: notification-service â 4 passive idempotent consumers (6 events), notification log + feed API, NotificationChannel/StubPushChannel (NOTF-01..06)
- [x] 05-07-PLAN.md â Wave 2: admin-service â dual-source append-only audit log (mutations + domain-event consumer) + ADMIN viewer (ADMN-06)
- [x] 05-08-PLAN.md â Wave 3: admin-web â httpOnly cookie login + middleware + shell/sidebar + user search + ledger screens (ADMN-01/02/03 UI)
- [x] 05-09-PLAN.md â Wave 4: admin-web â sender-ID queue, manual refund, bundle catalog, audit-log screens (ADMN-04/05/06/07 UI)
**UI hint**: yes
...
**Success Criteria** (what must be TRUE):
  ...
  3. User can purchase an SMS bundle via Azampay â the STK push countdown (2-minute timer) displays correctly and the balance updates automatically after confirmation
...
- [x] 06-01-PLAN.md â Wave 1: Flutter scaffold + l10n (EN+SW) + Dio QueuedInterceptor + go_router guards + Hive/secure-storage + Riverpod auth state + test harness (MOBL placeholder map)
- [x] 06-02-PLAN.md â Wave 1: backend D-11 â GET /api/v1/payments/{id} owner-scoped status endpoint (payment-service)
- [x] 06-03-PLAN.md â Wave 1: backend D-12 â campaign contactIds[] targeting (messaging + contact-service recipients-by-ids)
- [x] 06-04-PLAN.md â Wave 1: backend D-13 GET /auth/me + D-14 PATCH notifications/{id}/read (identity + notification services)
- [x] 06-05-PLAN.md â Wave 2: splash + onboarding (MOBL-01) + register/NIDA + login session persistence (MOBL-03)
- [x] 06-06-PLAN.md â Wave 2: NIDA PENDING walled state + 10s /auth/me auto-poll â VERIFIED (MOBL-02, D-09)
- [x] 06-07-PLAN.md â Wave 3: dashboard + balance cache-read/online-write + recent campaigns + shared widget kit + NavigationBar (MOBL-04)
- [x] 06-08-PLAN.md â Wave 4: bundle catalog + STK 2-min countdown + payment status polling (MOBL-05)
- [x] 06-09-PLAN.md â Wave 4: flat contact list (cache) + online-only add + delete (MOBL-06)
- [x] 06-11-PLAN.md â Wave 4: notification feed 30s polling + client-derived unread badge (D-01)
- [x] 06-10-PLAN.md â Wave 5: campaign composer (contactIds send) + GSM-7/UCS-2 counter + history + detail (MOBL-07/08)
- [x] 06-12-PLAN.md â Wave 6: e2e integration test + signing + store metadata + CI + submission checkpoint (MOBL-09, autonomous:false)
**UI hint**: yes
...
**Scope note**: Clean-architecture refactor is **payment-service ONLY** at this phase â it is the canonical reference. Rollout to the other 7 services is explicitly deferred to later phases. The refactor MUST be behavior-neutral: no API contract, DB schema, event, or test-outcome changes â the existing payment-service test suite must stay green throughout.
...
**Success Criteria** (what must be TRUE when complete):
  ...
  2. The full payment-service test suite (unit + Testcontainers integration) passes unchanged after the refactor â no behavior, contract, schema, or event change
...
**Execution Order:**
Phase 0 runs as a parallel background track. Coding phases execute: 1 â 2 â 3 â 4 â 5 â 6
...
**All 76 v1 requirements mapped (note: REQUIREMENTS.md header states 68 but the traceability table enumerates 76 distinct IDs â this roadmap maps all 76):**

| Phase | Requirements |
|-------|-------------|
| Phase 0 | (none â external actions) |
...
**Total mapped: 76 / 76** (v1 requirements; Phase 7 adds 4 post-v1.0 dev-experience requirements â ARCH-01, DEVX-01, DEVX-02, DEVX-03)
